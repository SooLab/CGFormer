...
class MultiModalSwinTransformer(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            load_checkpoint(self, pretrained, strict=('upernet' in pretrained), logger=logger_)
            logger.info('loading swin success !!!')
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.pwam_fusion = PWAM(dim,  # both the visual input and for combining, num of channels
                           dim,  # v_in
                           768,  # l_in
                           dim,  # key
                           dim,  # value
        ...
        #self.pwam_lattn = SelfAttn(qkv_dim=768, hidden_dim=768, out_dim=768, bias=False, attn_drop=.1, proj_drop=.1)

        self.pwam_gate = nn.Sequential(
        ...
        nn.init.zeros_(self.pwam_gate[0].weight)
        nn.init.zeros_(self.pwam_gate[2].weight)
    ...
    def forward(self, x, H, W, l, l_mask):
        ...
        # PWAM fusion
        x_residual = self.pwam_fusion(x, l, l_mask)
        # apply a gate on the residual
        x = x + (self.pwam_gate(x_residual) * x_residual)
...
class DynamicAdapter(nn.Module):
    def __init__(self, in_dim=512, out_dim=512, kernel_size=1):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.kernel_size = kernel_size
        self.adapter = nn.Linear(in_dim, in_dim*out_dim*kernel_size*kernel_size+out_dim)
    ...
    def forward(self, x, text):
        ...
        x = x.reshape(1, -1, 1, 1)
        # b c h w -> b c*c h w
        text = rearrange(text, 'b c n -> (b n) c')
        adapter = self.adapter(text) # Eq. 8
        weight, bias = adapter[:, :-self.out_dim], text[:, -self.out_dim:]
        weight = weight.reshape(-1, C, self.kernel_size, self.kernel_size)
        # print(x.shape, weight.shape, bias.shape)
        bias = bias.reshape(-1)
        # 1 (b n c) 1 1 -> 1 (b n c) 1 1
        out = F.conv2d(x,
                    weight,
                    padding=0,
                    groups=B*N,
        ...
        out = rearrange(out.squeeze(), '(b n c) -> b n c', b=B, c=self.out_dim, n=N)
